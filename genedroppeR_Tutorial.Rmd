...
unicorn.MHC.UF <- genedrop_multi_warbler(id = unicorn$id,
                                 mother = unicorn$mother,
                                 father = unicorn$father,
                                 cohort = unicorn$cohort,
                                 genotype = unicorn$MHC,
...
x <- genedrop_freq(id = unicorn$id,
                   mother = unicorn$mother,
                   father = unicorn$father,
                   allele_ids = c("A", "B"),
                   founder_allele_freqs = c(0.01, 0.99),
                   nsim = 1,
                   interval = 20)

summary_cohort(id = x$ID, cohort = x$cohort, genotype = x$Simulated.Geno)

head(unicorn)

x <- subset(x, select = c(ID, Simulated.Geno))
names(x) <- c("id", "SNP4")
unicorn <- join(unicorn, x)
unicorn$SNP4[which(unicorn$SNP4 == "BA")] <- "AB"
table(unicorn$SNP4)
...
x1 <- summary_genedrop(x)

plot_genedrop_results(x1)

plot_genedrop_lm_slopes(x1)

plot_genedrop_cumulative_change(x1)
```
