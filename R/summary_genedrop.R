...
summary_genedrop <- function(genedrop_obj) {
  ...
  cat("\n\n")
  cat(paste0(
    "Model parameters: cohorts = ", length(unique(genedrop_obj$observed_frequencies$Cohort)),
    ", Simulations = ", max(genedrop_obj$simulated_frequencies$Simulation),
    ", Founder cohorts = ", genedrop_obj$n_founder_cohorts, "."
  ))
  cat("\n")
  cat(paste0(
    "Founder genotypes ", ifelse(genedrop_obj$fix_founders, "fixed", "not fixed"),
...
process_genedrop <- function(genedrop_obj,
  ...
  if (is.numeric(genedrop_obj$Simulated.Geno)) {
    ...
    x3 <- filter(genedrop_obj, Simulation == 1) %>%
      select(True.Geno, cohort) %>%
      na.omit()
    x3 <- data.frame(
      Sum = tapply(x3$True.Geno, x3$cohort, sum),
