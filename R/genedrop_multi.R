...
genedrop_multi <- function(id,
  ...
  y$Allele1 <- sapply(y$genotype, function(foo) strsplit(foo, split = genotype_delim, fixed = T)[[1]][1])
  y$Allele2 <- sapply(y$genotype, function(foo) strsplit(foo, split = genotype_delim, fixed = T)[[1]][2])
