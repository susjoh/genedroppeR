...
#
# x <- locus_summary(ped$ID, ped$MOTHER, ped$FATHER, ped$cohort, ped$genotype)
# x <- na.omit(x)
#
...
#   temp$AlleleMOTHER <- sapply(temp$genotype, function (bar) strsplit(bar, split = "")[[1]][1])
#   temp$AlleleFATHER <- sapply(temp$genotype, function (bar) strsplit(bar, split = "")[[1]][2])
#
# } else {
#
